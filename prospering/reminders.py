...
class ReminderCog(commands.Cog):
    ...
    async def set_reminder(self, ctx, *, reminder_text: str):
        ...
        try:
            ...
            if not match:
                await ctx.send("Invalid format! Please use: !remindme <task name> <YYYY-MM-DD> <HH:MM>")
                return
            ...
            async with self.bot.db_pool.acquire() as conn:
                await conn.execute('''
                    ...
                    VALUES ($1, $2, $3, $4)
                ''', ctx.author.id, ctx.channel.id, task_name, reminder_time)
            
            await ctx.send(f"✅ I'll remind you about '{task_name}' on {date_str} at {time_str}")
    ...
    async def cancel_reminder(self, ctx, reminder_id: int):
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            ''', reminder_to_delete['id'], ctx.author.id)
